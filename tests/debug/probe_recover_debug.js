...
async function run(){
  ...
  try{
    ...
    try{
      ...
      console.log('getDbInstance returned db');
      console.log('provider_connections row:', db.prepare('SELECT id, provider, name FROM provider_connections WHERE id = ?').get('recover-openai'));
    } catch (err){
      console.error('getDbInstance threw:', err);
      // if preserved snapshot file exists, inspect it
      const dir = path.dirname(sqliteFile);
      const backups = fs.readdirSync(dir).filter((n)=>n.startsWith(path.basename(sqliteFile)+'.probe-failed-')).map(n=>path.join(dir,n));
      console.log('backups:', backups);
      if(backups.length>0){
        const snapshot = core.captureCriticalDbStateForTesting(backups[0]);
