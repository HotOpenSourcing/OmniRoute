...
export function classifyProviderError(
  ...
  // Upstream explicitly returned a "model produced no output" error.
  // Detected across all status codes (Freebuff returns this inside a 200 SSE
  // stream that closes with the error message, but sometimes upstream sends
  // 500/502 with the same message). Non-retryable, must check before generic
