...
# -*- coding: utf-8 -*-
"""
Test complet des 78 modèles Windsurf avec compte PRO.
Utilise le port 51834 (l.localhost) et le nouveau token CSRF.
"""
import json
import sys
import os
...
# Configuration
os.environ['WINDSURF_LS_PORT'] = '51834'

def test_model_identity(model_name, token):
        ...
        if body_hex:
            try:
                ...
                if text_match:
                    ...
                    if 'I am' in body_text or 'I\'m' in body_text:
                        # Extraire la première phrase avec "I am" ou "I'm"
                        match = re.search(r'(I am [^.!?]+[.!?]|I\'m [^.!?]+[.!?])', body_text)
                        if match:
...
def main():
    ...
    print('='*70)
    print('TEST DE TOUS LES MODÈLES WINDSURF - COMPTE PRO')
    ...
    print(f'✅ Port: 51834 (l.localhost)')
    ...
    for idx, model in enumerate(all_models, 1):
        ...
        result = test_model_identity(model, token)
        ...
        if result['status'] == 'success':
            ...
            response_preview = result["response"][:100] if result["response"] else "No response"
            print(f'    Réponse partielle: {response_preview}...')
        else:
            print(f'❌')
    ...
    output_file = 'windsurf_all_models_identity_test_pro.json'
